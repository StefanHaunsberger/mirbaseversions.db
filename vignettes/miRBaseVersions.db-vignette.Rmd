...
mature miRNA names from 23 miRBase release versions (6.0 through 22.1).
Due to ongoing growth and
...
miRBase 22.1 only updated the high-confidence subset (which this package
does not model); sequences, accessions and names are unchanged from v22.0.
The `VW-MIMAT-22.1` keytype is therefore a relabelled alias view over
`VW-MIMAT-22.0`. Queries through it return the same rows as v22.0 with the
`VERSION` column set to `22.1`.
