# This function populates the version sqlite table
#	It either takes an input file (tab separated text file) or a df
# 
# Author: stefanhaunsberger
###############################################################################

#source("src/miRNANameConverter/processVersionNumbers.R");

insertVersions = function(input.file, df, db, uri, verbose = TRUE) {
...
#	}
	...
	if (all(is.null(args$input.file), is.null(args$df))) {
		...
		input.file = file.path(input.path, "versions.txt");
...
#	dbSendQuery(db, "DELETE FROM organism");
	...
	if (!all(colnames(df) %in% c("number", "information"))) {
		stop("Only 'number' and 'information' column accepted.");	
	}
	
	writeLines(sprintf("Inserting %i records into table 'versions'...", nrow(df)));
	# Populate table
	dbWriteTable(conn = db, name = "version", value = df, row.names = FALSE, overwrite = TRUE);
