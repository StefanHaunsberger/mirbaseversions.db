# This function populates the mimat sqlite table
...
insertMIMAT = function(input.file, df, db, uri, verbose = TRUE) {
...
#	}
	...
	if (all(is.null(args$input.file), is.null(args$df))) {
		...
		input.file = file.path(input.path, "mi-data.txt");
...
#	dbSendQuery(db, "DELETE FROM organism");
	...
	if (!all(colnames(df) == dbListFields(db, "mimat"))) {
		stop("Columns names don't match");
	}
	
	writeLines(sprintf("Inserting %i records into table 'mimat'...", nrow(df)));
	# Populate table
	dbWriteTable(conn = db, name = "mimat", value = df, row.names = FALSE, overwrite = TRUE);
